...
This solution utilizes an automated bash script to complete all tasks sequentially, including downloading the application source, creating the Dockerfile, building and pushing the image to Artifact Registry, and deploying it to the Kubernetes cluster.

### Execution Instructions

1. Open **Google Cloud Shell** in your lab environment.
2. Ensure you have the **Region** and **Zone** provided in your Qwiklabs instruction panel ready.
3. Create a new shell script file using the `nano` editor:
```bash
nano solution.sh
...
5. Save and exit by pressing **CTRL+O**, **ENTER**, and **CTRL+X**.
...
8. The script will prompt you for variables. Press **Enter** to accept the default values for Repository Name, Docker Image Name, and Tag. When prompted for **Region** and **Zone**, type the specific values assigned to your lab session and press **Enter**.
...
echo
echo "${CYAN}Starting Lab Execution...${RESET}"
echo
...
echo "${CYAN}Verifying image in Artifact Registry...${RESET}"
...
Wait for the script to finish running and output the external IP of the service. Once complete, click **Check my progress** on all tasks in the Qwiklabs instruction panel to verify your score.
